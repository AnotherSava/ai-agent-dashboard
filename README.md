...
Anything that can POST JSON to localhost can report its status. Each session appears as a row in a compact always-on-top window, with a color-coded icon identifying the source tool and a live badge that transitions between idle / working / awaiting / done / error.
