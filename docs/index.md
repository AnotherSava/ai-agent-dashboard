...
Anything that can POST JSON to localhost can report its status. Each session appears as a row in a compact always-on-top window, with a color-coded icon identifying the source tool and a live status badge that transitions between idle / working / awaiting / done / error.
...
3. The widget shows one row per active session. Color-coded dots animate while `working` / `awaiting`, settle green on `done`, flash red on `error`.
